...
-- Create policies for voltmarket_nda_requests
CREATE POLICY "Users can submit NDA requests" ON public.voltmarket_nda_requests 
FOR INSERT WITH CHECK (auth.uid() = requester_id);

CREATE POLICY "Users can view NDA requests they're involved in" ON public.voltmarket_nda_requests 
FOR SELECT USING (auth.uid() = requester_id OR auth.uid() IN (
  SELECT user_id FROM gridbazaar_profiles WHERE id = seller_id
));

CREATE POLICY "Sellers can update NDA request status" ON public.voltmarket_nda_requests 
